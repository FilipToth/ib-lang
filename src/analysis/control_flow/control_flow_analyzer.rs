...
use crate::analysis::{binding::types::TypeKind, error_bag::{ErrorBag, ErrorKind}, CodeLocation};

use super::control_flow_graph::ControlFlowNode;

fn analyze_func_rec(node: Rc<RefCell<ControlFlowNode>>, loc: &CodeLocation, func_ret_type: &TypeKind, errors: &mut ErrorBag) {
    ...
    if let Some(next) = &node.next {
        ...
        let next_ref = next.clone();
         ...
         if next_node.is_end {
            ...
            let kind = ErrorKind::ReturnTypeMismatch { found: ret_type.clone(), expected: func_ret_type.clone() };
...
pub fn analyze_func(root: Rc<RefCell<ControlFlowNode>>, loc: &CodeLocation, func_ret_type: &TypeKind, errors: &mut ErrorBag) {
